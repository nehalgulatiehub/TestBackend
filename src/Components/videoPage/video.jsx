...
function Video({video, videoId}) {
  ...
  const addtoWatchHistory = async () => {
    // TODO: Implement watch history and view count endpoints in backend
    // try {
    //   const resHistory = await axiosInstance.post(`/videos/watch-history/v/${historyId}`);
    //   const resView = await axiosInstance.post(`/videos/view/v/${historyId}`);
    // }
    // catch(error) {
    //   console.log(error);
    // }
    ...
    <div className="flex w-full flex-wrap gap-4 p-4 lg:flex-nowrap">
        ...
        <div className="col-span-12 flex w-full shrink-0 flex-col gap-3 lg:w-[350px] xl:w-[400px]">
          {video.allVideos && video.allVideos.map((videoDetails) => {
            const owner = videoDetails.owner || videoDetails.ownerDetails;
            const avatar = owner?.avatar?.url || (typeof owner?.avatar === 'string' ? owner.avatar : '');
            const thumbnail = videoDetails.thumbnail?.url || (typeof videoDetails.thumbnail === 'string' ? videoDetails.thumbnail : '');
            return (
              <RelatedVideoCard 
                ...
                views={videoDetails.views || videoDetails._v || 0} 
              />
            );
          })}
        </div>
