...
function DashBoardUploadingVideo({handleCloseVideo, uploaded, name, videoSize}) {
    return (
        <div className="absolute inset-x-0 top-0 z-10 mt-24 flex h-[calc(100vh-66px)] items-center justify-center bg-black/50 px-4 pb-[86px] pt-4 sm:h-[calc(100vh-82px)] sm:px-14 sm:py-8">
        <div className="w-full max-w-lg overflow-auto rounded-lg border border-gray-700 bg-[#121212] p-4">
          <div className="mb-4 flex items-start justify-between">
            <h2 className="text-xl text-gray-200 font-semibold">
              {uploaded ? 'Upload Complete!' : 'Uploading Video...'}
              <span className="block text-sm text-gray-300">
                {uploaded ? 'Your video has been uploaded successfully.' : 'Track your video uploading process.'}
              </span>
            </h2>
            <button onClick={handleCloseVideo} className="h-6 text-gray-400 w-6">
              <svg
          ...
          <div className="mb-6 flex gap-x-2 border p-3">
            ...
            <div className="flex flex-col">
              <h6 className='text-gray-200'>{name}</h6>
              <p className="text-sm text-gray-200">{videoSize}</p>
              <div className="mt-2 flex items-center">
                {/* FIXED: Inverted the logic - show spinner while uploading */}
                {!uploaded ? (
                  <svg
                    aria-hidden="true"
                    role="status"
                ...
                {/* FIXED: Inverted the text logic */}
                <span className='text-white'>{!uploaded ? 'Uploading...' : 'Uploaded successfully'}</span>
              </div>
            </div>
          </div>
          <div className="grid grid-cols-1 gap-4">
            {/* FIXED: Added onClick handler and inverted disabled logic */}
            <button
              onClick={handleCloseVideo}
              className="bg-[#ae7aff] w-full px-4 py-3 text-black font-semibold disabled:bg-[#E4D3FF] disabled:cursor-not-allowed hover:bg-[#9c6cee]"
              disabled={!uploaded}>
              Finish
            </button>
          </div>
...
export default DashBoardUploadingVideo
